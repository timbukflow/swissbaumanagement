<!DOCTYPE html>
<html lang="de">
<head prefix="og: http://ogp.me/ns# fb: http://ogp.me/ns/fb# place: http://ogp.me/ns/place#">
    <meta charset="utf-8">
    <title>Swiss Baumanagement | Neubau MFH Goldach</title>
    <meta name="description" content="Neubau, Mehrfamilienhaus MFH, Goldach">
    <?php include 'head.php'; ?>
</head>
  
<body>
    <?php include 'header.php'; ?>
    <section class="container">
        <div class="gridproject">
            <div class="imgproject">
                <div class="inview"></div>
                <img src="img/swiss-baumanagement-neubau-mfh-goldach-01.jpg" alt="swiss-baumanagement-neubau-mfh-goldach" >
            </div>
        </div>
        <div class="gridprojectinfo">
            <h1>
            Neubau MFH <br> Goldach
            </h1>
            <h2>
                <ul>
                    <li class="overtitle">Adresse:</li>
                    <li>9403 Goldach</li>
                    <li class="overtitle">Projektumfang:</li>
                    <li>Neubau, Mehrfamilienhaus mit Tiefgarage (11 Wohnungen)</li>
                    <li class="overtitle">Verfahren:</li>
                    <li>Auftrag Investor</li>
                    <li class="overtitle">Ausführung:</li>
                    <li>2020 - 2021</li>
                    <li class="overtitle">Leistung:</li>
                    <li>Projekt + Werkplanung</li>
                </ul>
            </h2>
        </div>
        <div class="grid30">
            <div class="gridcontainer">
                <div class="imgproject">
                    <div class="inview"></div>
                    <img src="img/swiss-baumanagement-neubau-mfh-goldach-02.jpg" alt="swiss-baumanagement-neubau-mfh-goldach" >
                </div>
            </div>
        </div>
        <div class="grid30">
            <div class="gridcontainer">
                <div class="imgproject">
                    <div class="inview"></div>
                    <img src="img/swiss-baumanagement-neubau-mfh-goldach-03.jpg" alt="swiss-baumanagement-neubau-mfh-goldach" >
                </div>
            </div>
        </div>
        <div class="grid30">
            <div class="gridcontainer">
                <div class="imgproject">
                    <div class="inview"></div>
                    <img src="img/swiss-baumanagement-neubau-mfh-goldach-04.jpg" alt="swiss-baumanagement-neubau-mfh-goldach" >
                </div>
            </div>
        </div>   
    </section>
    <div class="nextproject">
        <a href="umbau-neubau-mfh-kreuzlingen" target="_top">
            <h2>Nächstes Projekt</h2>
            <img class="pfeilnext" src="img/pfeil.svg" width="30px" height="24px">
        </a>
    </div>   
    
    <?php include 'footer.php'; ?>    
    <?php include 'script.php'; ?>
</body>
</html>
